...
namespace DynamicProxyDemo
{
    ...
    public class ProxyFactory
    {
        ...
        public object Create(IInvocationHandler handler, Type objType)
        {
            return Create(handler, objType, false);
        }

        private Type CreateType(IInvocationHandler handler, Type[] interfaces, string dynamicTypeName)
        {
                ...
                for (var i = 0; i < interfaceMethods.Length; i++)
                {
                    ...
                    methodIl.Emit(OpCodes.Call,
                        typeof(MetaDataFactory).GetMethod(
                            "GetMethod", new[] {typeof(string), typeof(int)}));
